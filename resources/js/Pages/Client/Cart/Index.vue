<template>
    <article class="w-full max-w-6xl mx-auto px-8 py-6 bg-white rounded shadow-sm">
        <template v-if="this.$page.props.auth.user.carts_total_sum > 0">
            <div class="mb-4">
                <table class="min-w-full bg-white shadow-md rounded-lg overflow-hidden">
                    ...
                    <tr v-for="cart in cartsData">
                        ...
                        <td class="px-6 py-4 border-b border-gray-200 text-gray-800">
                            <svg @click.prevent="destroyCart(cart)" xmlns="http://www.w3.org/2000/svg" fill="none"
            ...
            <div class="mb-4">
                <div class="text-right text-gray-600 font-semibold uppercase">
                    <h3>Итого: {{ this.$page.props.auth.user.carts_total_sum }}</h3>
                    <button type="submit"
        ...
        <template v-else>
            <div class="mb-4 bg-white text-gray-600 font-semibold uppercase text-center">
                Корзина пуста, добавьте товар из <Link :href="route('client.categories.index')" class="text-indigo-400">категорий</Link>
            </div>
        </template>
    </article>
</template>
